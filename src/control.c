...
static void init_odrive(const struct device *pd) {
  const char calibration[] = "w axis0.requested_state 3\n";
  const char loop_control[] = "w axis0.requested_state 8\n";
  pd_write_cmd(pd, calibration, sizeof(calibration) - 1);
  pd_write_cmd(pd, loop_control, sizeof(loop_control) - 1);
}

static void set_torque(const struct device *pd, float torque) {
  char buf[128];
  snprintf(buf, sizeof(buf), "c 0 %f\n", (double)torque);
  pd_write_cmd(pd, buf, strlen(buf));
}

static void set_max_vel(const struct device *pd, float value) {
  char buf[128];
